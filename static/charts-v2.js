...
const I18N = {
  ...
  zh: {
    ...
    classicUi: "← 经典界面", fioriUi: "Fiori 界面 →",
    theme: "主题 ▾",
    themeAmber: "琥珀终端",
    themeArctic: "北极",
    themeEmerald: "翠绿暗夜",
  ...
  zht: {
    ...
    classicUi: "← 經典介面", fioriUi: "Fiori 介面 →",
    theme: "主題 ▾",
    themeAmber: "琥珀終端",
    themeArctic: "北極",
    themeEmerald: "翠綠暗夜",
...
function applyLang() {
  ...
  // Highlight active language button (v2 uses .lang-active class)
  ["langEn", "langZh", "langZht"].forEach(id => {
    const btn = document.getElementById(id);
    if (!btn) return;
    const code = { langEn: "en", langZh: "zh", langZht: "zht" }[id];
    btn.classList.toggle("lang-active", currentLang === code);
    btn.disabled = currentLang === code;
  });
...
function setRange(days) {
  ...
  document.getElementById("toDate").value = toLocalDate(to);
  document.getElementById("fromDate").value = toLocalDate(from);
  document.querySelectorAll("#qrAll,#qr7,#qr30,#qr90,#qr180,#qr365,#qrThisWeek,#qrThisMonth,#qrThisYear,#qrLastWeek,#qrLastMonth,#qrLastYear").forEach(b => b.classList.remove("active"));
  ...
  afterDatasetsDraw(chart) {
    ...
    chart.data.datasets.forEach((dataset, di) => {
      ...
      [{ idx: maxIdx, val: max, above: true }, { idx: minIdx, val: min, above: false }].forEach(({ idx, val, above }) => {
        ...
        ctx.fillStyle = above ? "#22c55e" : "#ef4444";
        ctx.beginPath();
...
// ── Dark terminal colour palette ───────────────────────────────────────────────
const V2_COLORS = {
  bg:       "#181b22",
  grid:     "#252a36",
  tick:     "#8a96b4",
  tooltip:  { bg: "#13151a", border: "#2e3447", text: "#e8eaf0" },
  legend:   "#b0b8d0",
};

// Override Chart.js defaults for dark background
Chart.defaults.color           = V2_COLORS.tick;
Chart.defaults.borderColor     = V2_COLORS.grid;
Chart.defaults.backgroundColor = V2_COLORS.bg;
...
const chartOptions = {
  ...
  plugins: {
    tooltip: {
      mode: "index",
      intersect: false,
      backgroundColor: V2_COLORS.tooltip.bg,
      borderColor:     V2_COLORS.tooltip.border,
      borderWidth:     1,
      titleColor:      "#f5a623",
      bodyColor:       V2_COLORS.tooltip.text,
      padding:         10,
  ...
  scales: {
    x: {
      grid:  { color: V2_COLORS.grid },
      ticks: { maxTicksLimit: 8, color: V2_COLORS.tick, font: { family: "'Space Mono', monospace", size: 10 } },
      border: { color: V2_COLORS.grid },
    },
    y: {
      grid:  { color: V2_COLORS.grid },
      ticks: { callback: v => v.toFixed(2), color: V2_COLORS.tick, font: { family: "'Space Mono', monospace", size: 10 } },
      border: { color: V2_COLORS.grid },
    }
  },
...
function renderMergedChart(canvasId, datasets) {
  ...
  charts[canvasId] = new Chart(document.getElementById(canvasId), {
    ...
    options: { ...chartOptions, plugins: { ...chartOptions.plugins, legend: { display: true, position: "top", labels: { color: V2_COLORS.legend, font: { family: "'Space Mono', monospace", size: 10 }, boxWidth: 12, padding: 12 } } } }
  });
}
...
function setCols(n) {
  ...
  document.querySelectorAll(".col-picker button, #col1, #col2, #col3, #col4").forEach(b => b.classList.remove("active"));
...
const THEMES = {
  amber: {
    bg: "#181b22", grid: "#252a36", tick: "#8a96b4",
    tooltip: { bg: "#13151a", border: "#2e3447", text: "#f0f2f8", title: "#f5a623" },
    legend: "#b0b8d0",
  },
  arctic: {
    bg: "#ffffff", grid: "#d4d8e8", tick: "#64748b",
    tooltip: { bg: "#ffffff", border: "#bcc3d8", text: "#0f172a", title: "#2563eb" },
    legend: "#334155",
  },
  emerald: {
    bg: "#0e1912", grid: "#1a2e20", tick: "#4d7c5f",
    tooltip: { bg: "#0a120e", border: "#213d29", text: "#ecfdf5", title: "#10b981" },
    legend: "#a7f3d0",
  },
};
...
function applyThemeToCharts(theme) {
  ...
  // Rebuild chartOptions scales/tooltip in-place
  chartOptions.scales.x.grid.color       = T.grid;
  chartOptions.scales.x.ticks.color      = T.tick;
  chartOptions.scales.x.border.color     = T.grid;
  chartOptions.scales.y.grid.color       = T.grid;
  chartOptions.scales.y.ticks.color      = T.tick;
  chartOptions.scales.y.border.color     = T.grid;
  ...
  // Update legend color option used in renderMergedChart
  V2_COLORS.legend = T.legend;
  V2_COLORS.bg     = T.bg;
}
...
function setTheme(name) {
  ...
  if (name !== "amber") document.body.classList.add("theme-" + name);
  localStorage.setItem("v2theme", name);
  // Update active button
  document.querySelectorAll(".theme-opt").forEach(b => b.classList.remove("active"));
  const btn = document.getElementById("theme" + name.charAt(0).toUpperCase() + name.slice(1));
  if (btn) btn.classList.add("active");
  // Close menu
  document.getElementById("themeDropdown").style.display = "none";
  // Re-apply chart defaults and re-render
  applyThemeToCharts(name);
...
const _savedTheme = localStorage.getItem("v2theme") || "amber";
applyThemeToCharts(_savedTheme);
if (_savedTheme !== "amber") document.body.classList.add("theme-" + _savedTheme);
const _themeBtn = document.getElementById("theme" + _savedTheme.charAt(0).toUpperCase() + _savedTheme.slice(1));
if (_themeBtn) { document.querySelectorAll(".theme-opt").forEach(b => b.classList.remove("active")); _themeBtn.classList.add("active"); }
...
document.addEventListener("click", e => {
  ...
  if (!document.getElementById("themeMenu").contains(e.target))
    document.getElementById("themeDropdown").style.display = "none";
});
loadAll();

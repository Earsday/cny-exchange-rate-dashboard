...
- **Column picker** (1/2/3 cols) — sets `grid-template-columns` on all `.chart-grid` elements; destroys and re-renders charts to pick up new container width
- **Keep up-to-date button** — calls `POST /api/collect` to backfill any missing data, then reloads charts
- **Export button** — dropdown: "Separate images" (one PNG per visible chart) or "Combined image" (stitches all visible canvases onto an offscreen canvas respecting current column layout)
...
- **Resizable**: a 5px drag handle on the inner edge allows resizing between 13% and 50% of viewport width. The width is stored as a fraction so it scales proportionally on browser window resize. Charts in `#mainArea` call `c.resize()` via `setTimeout(..., 50)` after the sidebar opens/closes to reflow correctly.
